...
export function JourneyMaps({ 
  ...
  const [isSuggesting, setIsSuggesting] = useState<{ stageId: string, laneId: string } | null>(null);
  const [isRegeneratingStage, setIsRegeneratingStage] = useState<string | null>(null);
  ...
  const handleSummarizeJourney = async () => {
    ...
    try {
      ...
      if (!hasKey) {
        addToast("Gemini API key is required for AI summary.", "error");
      ...
      const journeyData = activeJourney.stages.map(s => ({
        stage: s.name,
        emotion: s.emotion,
        items: Object.entries(s.laneData).map(([laneId, items]) => ({
          lane: activeJourney.swimlanes.find(l => l.id === laneId)?.name || laneId,
          items: items.map(i => typeof i === 'string' ? i : i.title)
        }))
      }));

      const prompt = `You are an expert CX strategist. Provide a high-level executive summary of the following customer journey map.
      Journey Title: ${activeJourney.title}
      
      Journey Data:
      ${JSON.stringify(journeyData, null, 2)}
      
      Provide a concise summary (max 200 words) highlighting:
      1. The overall customer sentiment trend.
      2. The biggest friction points.
      3. The most impactful opportunities for improvement.
      ...
      setJourneySummary(result.text || "No summary generated.");
      addToast("Journey summary generated", "success");
    } catch (err: any) {
      console.error(err);
  ...
  const handleSuggestItems = async (stageId: string, laneId: string) => {
    if (!activeJourney) return;
    setIsSuggesting({ stageId, laneId });
    
    try {
      const hasKey = await ensureApiKey();
      if (!hasKey) {
        addToast("Gemini API key is required for AI suggestions.", "error");
      ...
      const prompt = `You are an expert CX designer. Suggest 3-5 items for a customer journey map.
      Journey Title: ${activeJourney.title}
      Stage: ${stage.name}
      Swimlane: ${lane.name}
      
      Context: This is a ${activeJourney.state} state journey map.
      
      Provide a JSON object with a 'suggestions' array of strings. Each string should be a concise, actionable item (max 10 words).`;
      ...
      if (suggestions.length > 0) {
        const updatedStages = activeJourney.stages.map(s => {
          if (s.id === stageId) {
            ...
            return {
              ...
              laneData: {
                ...s.laneData,
                [laneId]: [...currentItems, ...suggestions]
              }
            };
          }
          return s;
        ...
        setJourneys(journeys.map(j => j.id === activeJourney.id ? { ...j, stages: updatedStages } : j));
        addToast(`Added ${suggestions.length} suggestions to ${stage.name} - ${lane.name}`, "success");
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleRegenerateStage = async (stageId: string) => {
    if (!activeJourney) return;
    setIsRegeneratingStage(stageId);
    
    try {
      const hasKey = await ensureApiKey();
      if (!hasKey) {
        addToast("Gemini API key is required for AI regeneration.", "error");
        setIsRegeneratingStage(null);
        return;
      }
      ...
      const prompt = `You are an expert CX designer. Regenerate the content for a specific stage in a customer journey map.
      Journey Title: ${activeJourney.title}
      Stage to Regenerate: ${stage.name}
      
      Provide data for these specific swimlanes:
      - lane_touchpoints: What the customer interacts with
      - lane_friction: Pain points or frustrations
      - lane_opportunities: Ideas to improve the experience
      - lane_backoffice: Internal processes
      - lane_teams: Internal teams involved
      - lane_systems: Technology or software systems used
      
      Also provide an emotion score from 1 to 5.
      
      Provide a JSON object with the stage data.`;
      ...
      const result = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            ...
            required: ['emotion', 'lane_touchpoints', 'lane_friction', 'lane_opportunities', 'lane_backoffice', 'lane_teams', 'lane_systems']
          }
        }
      });
      ...
      const updatedStages = activeJourney.stages.map(s => {
        if (s.id === stageId) {
          return {
            ...
            laneData: {
              'lane_touchpoints': data.lane_touchpoints || [],
              'lane_friction': data.lane_friction || [],
              'lane_opportunities': data.lane_opportunities || [],
              'lane_backoffice': data.lane_backoffice || [],
              'lane_teams': data.lane_teams || [],
              'lane_systems': data.lane_systems || []
            }
          };
        }
        return s;
      ...
      setJourneys(journeys.map(j => j.id === activeJourney.id ? { ...j, stages: updatedStages } : j));
      addToast(`Regenerated ${stage.name} stage content`, "success");
    } catch (err: any) {
      console.error(err);
      addToast(err.message || "Failed to regenerate stage", "error");
    } finally {
      setIsRegeneratingStage(null);
    }
  };
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          {activeJourneyId && (
            <div className="flex items-center gap-2 bg-white dark:bg-zinc-900 p-1 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
              <button 
                onClick={handleSummarizeJourney}
                disabled={isSummarizing}
                className="hover:bg-indigo-50 dark:hover:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 px-3 py-1.5 rounded-lg font-medium flex items-center gap-2 transition-all text-sm disabled:opacity-50"
                title="Summarize Journey with AI"
              >
                ...
                >
                  <div className="flex flex-col items-center justify-center gap-2">
                    ...
                    <div className="flex items-center justify-center gap-2 w-full">
                      ...
                      />
                      {canEdit && (
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity no-export shrink-0">
                          <button 
                            onClick={() => handleRegenerateStage(stage.id)} 
                            disabled={isRegeneratingStage === stage.id}
                            className="text-zinc-300 hover:text-indigo-500 transition-colors"
                            title="Regenerate Stage with AI"
                          >
                            {isRegeneratingStage === stage.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                          </button>
                          <button onClick={() => removeStage(stage.id)} className="text-zinc-300 hover:text-rose-500 transition-colors">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          {canEdit && (
                            <div className="mt-1 flex items-center gap-2">
                                ...
                                <button 
                                  onClick={() => handleSuggestItems(stage.id, lane.id)}
                                  disabled={isSuggesting?.stageId === stage.id && isSuggesting?.laneId === lane.id}
                                  className={`text-xs text-indigo-500 hover:text-indigo-700 font-medium flex items-center gap-1 print:hidden no-export disabled:opacity-50`}
                                  title="Suggest items with AI"
                                >
                                  {isSuggesting?.stageId === stage.id && isSuggesting?.laneId === lane.id ? (
            ...
            >
              <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-bold text-zinc-900 dark:text-white text-lg">AI Journey Summary</h3>
                </div>
                <button 
              ...
              <div className="p-8 overflow-y-auto custom-scrollbar">
                <div className="prose prose-sm dark:prose-invert max-w-none">
                  <ReactMarkdown>{journeySummary}</ReactMarkdown>
                </div>
              </div>
              ...
              <div className="p-6 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex justify-end">
                <button 
                  onClick={() => setJourneySummary(null)}
                  className="px-6 py-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl font-bold hover:opacity-90 transition-all"
                >
                  Close
                </button>
              </div>
